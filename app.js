...
App({
  ...
  server_api_3: (function (protocol) {
    return {
      defaultActivity: protocol + '/activity/experience_1/service/mini_apps/access_log/add',//埋点
      get_user_info: protocol + "/activity/experience_1/service/mini_apps/user_info/get",//获取用户信息
      get_mobile: protocol + "/activity/experience_1/service/mini_apps/mobile/get",//解密接口
      ...
      receiveCard: protocol + "/activity/experience_1/service/mini_apps/user_one/receive",//领取优惠券
      receiveCard2: protocol + "/activity/experience_1/service/mini_apps/user_two/receive",//在输入手机号页面领取优惠券
    }
  })(server),
  ...
  decryptedData_3: function (fn) {
    ...
    wx.getStorage({
      ...
      fail: function (failRes) {
        ...
        wx.request({
          method: "POST",
          url: that.server_api_3.get_user_info,
          data: {
            encrypt_data: that.user_info_data.encryptedData,
            code: that.login_data.code,
            iv: that.user_info_data.iv,
  ...
  //解密手机
  decryptedData_getMobile_3: function (fn) {
    var that = this;
    wx.request({
      method: "POST",
      url: that.server_api_3.get_mobile,
      data: {
        encrypt_data: that.user_info_data.encryptedData,
        code: that.login_data.code,
        iv: that.user_info_data.iv,
  ...
  //从缓存拿数据
  getStorage_user_info_data_3: function (fn) {
    var options;
    wx.getStorage({
      key: 'user_info_data_3',
      success: function (res) {
  ...
  initView_3: function (fn) {
    ...
    that.getStorage_user_info_data_3(function (options) {
      ...
      wx.request({
        method: "POST",
        url: that.server_api_3.init_view,
        data: options,
        success: function (res) {
          console.log('初始返回res=' + JSON.stringify(res.data));
          res = res.data;
  ...
  receiveCard_3: function (fn) {
    ...
    wx.request({
      method: "POST",
      url: that.server_api_3.receiveCard,
      data: options,
      success: function (res) {
        res = res.data;
        console.log('领卡返回res=' + JSON.stringify(res));
  ...
  //领卡的接口2
  receiveCard2_3: function (code, fn) {
    wx.showLoading({
      title: '领取中',
    })
    var that = this;
    ...
    wx.request({
      method: "POST",
      url: that.server_api_3.receiveCard2,
      data: options,
      success: function (res) {
        res = res.data;
        console.log('领卡返回res=' + JSON.stringify(res));
  ...
  //获取验证码
  getSmsCode_3: function (mobile, fn) {
    var that = this;
    wx.request({
      method: "POST",
      url: that.server_api_3.get_sms_code,
      data: {
        mobile: mobile,
        open_gid: that.group_data.open_gid,
        city_id: that.user_info_data.city_id
  ...
  //校验验证码
  checkSmsCode_3: function (mobile, sms_code, fn) {
    var that = this;
    wx.showLoading({
      title: '验证中',
    })
    wx.request({
      method: "POST",
      url: that.server_api_3.sms_code_check,
      data: {
        mobile: mobile,
        sms_code: sms_code,
        open_gid: that.group_data.open_gid,
