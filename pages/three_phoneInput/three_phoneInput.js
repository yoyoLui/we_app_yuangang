...
Page({
  ...
  onLoad: function (option) {
    //埋点-进入输入手机号和验证码人数 lBr4zF 
    app.defaultActivity_3('lBr4zF');
    var that = this;
  ...
  sendCode: function () {
    ...
    if ((/^1[0-9][0-9]\d{8}$/.test(that.data.mobile) && that.data.code_disabled == '')) {
      ...
      //发送验证码
      app.getSmsCode_3(that.data.mobile, function (res) {
  ...
  bindCodeInput: function (e) {
    ...
    if (sms_code.length >= 6 && that.data.isRightPhone && that.data.hasSendCode) {
      //校验验证码
      app.checkSmsCode_3(that.data.mobile, sms_code, function (res) {
        if (res.ret != 0) {//验证失败
